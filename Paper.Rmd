...
Methodology:
To start, we created functions that generate a random step length. This was done by using a normal distribution with a mean and standard deviation of the minimum of the dimensions divided by four and twelve respectively. Once we have a step length we generate a step direction from a uniform(0, 2$\pi$) distribution. This is the direction angle in radians. From these two pieces of information, we can then update the coordinate values in the update_coords function.
