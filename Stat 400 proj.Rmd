...
get_bootstrap_cis <- function(days, num_subjects, x_dim, y_dim, I_0, inf_dist, sample_size, ci_type='perc'){
  ...
  ci_df <- matrix(data=NA, nrow=days + 1, ncol=2)
  ...
  for(i in 1:ncol(samps)){
    ...
    ci_df[i, ] <- c(ci_obj$percent[4], ci_obj$percent[5])
    }else{ci_df[i, ] <- c(samps[1, i], samps[1, i])}
  }
return(ci_df)
}

get_bootstrap_cis(days=50, num_subjects=100, x_dim=10, y_dim=10, I_0=2, inf_dist=1, sample_size=5)
