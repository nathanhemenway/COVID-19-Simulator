...
starting_vals <- function(num_subjects, I_0, x_dim, y_dim){
  ...
  df <- as.data.frame(cbind(as.numeric(start_x), as.numeric(start_y), subjects, inc_time, rec_time))
  colnames(df)[1] <- 'x'
  colnames(df)[2] <- 'y'
  colnames(df)[4] <- 'inc_time'
  colnames(df)[5] <- 'rec_time'
  return(df)
}
...
infection_status <- function(state, mean_crit_dist){
  ...
  #store vector of newly infecteds
  newly_infected <- NULL
