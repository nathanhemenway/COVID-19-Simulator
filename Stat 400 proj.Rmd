...
starting_vals <- function(num_subjects, I_0, x_dim, y_dim){
  ...
  #Create null incubation and recovery time columns
  #inc_time <- rep(NA, num_subjects)
  #rec_time <- rep(NA, num_subjects)
  
  #pregenerated days of recovery times for each person
  recovery_days <- rpois(num_subjects,14)
  inc_time <- rpois(num_subjects,5)
  
  #Put into a data frame
  df <- as.data.frame(cbind(as.numeric(start_x), as.numeric(start_y), subjects,recovery_days,inc_time))
  colnames(df)[1] <- 'x'
  colnames(df)[2] <- 'y'
  colnames(df)[3] <- 'subjects'
  colnames(df)[4] <- 'recovery'
  colnames(df)[5] <- 'incubation'
  return(df)
}
...
#t = the current day, rt is df of recovery times. 
infection_status <- function(state, mean_crit_dist,t,rt){
  ...
  for(i in 1:nrow(current_infected)){
    for(j in 1:nrow(current_non_infected)){
      ...
      #day of recovery for current infected
      z_s <- as.numeric(rt[i])
      ...
      #check recovery time for person at z_s. If their recovery day time is past the current day, they recovered. 
      if(z_s>=t){
        new_status[j] <- 'Rec'
      }
  ...
  #current_non_infected[newly_infected, 4] <- rpois(n=length(newly_infected), lambda = 14)
  
  #Give a recovery time
  #current_non_infected[newly_infected, 5] <- rpois(n=length(newly_infected), lambda = 14)
...
simulator <- function(days, num_subjects, x_dim, y_dim, I_0){
  ...
  for(i in 1:days){
    ...
    if(i == 1){
      ...
      new_status <- infection_status(checked_positions, 2,i,recovery_time)
      ...
      new_status <- infection_status(updated_state, 2,i,recovery_time)
      ...
      new_status <- infection_status(checked_positions, 1/5,i,recovery_time)
      checked_positions[,3] <- new_status
      end_day[[i]] <- checked_positions
      ...
      new_status <- infection_status(updated_state, 1/5,i,recovery_time)
