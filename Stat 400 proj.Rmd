...
infection_status <- function(state, mean_crit_dist){
  ...
  for(i in 1:nrow(current_infected)){
    for(j in 1:nrow(current_non_infected)){
      ...
      x_s <- as.numeric(current_non_infected[j, 'x'])
      y_s <- as.numeric(current_non_infected[j, 'y'])
      
      crucial_distance <- rexp(1, 1/mean_crit_dist)
      d <- sqrt((x_i - x_s)^2 + (y_i - y_s)^2)
      print(c(d <= crucial_distance, x_i, x_s))
...
simulator <- function(days, num_subjects, x_dim, y_dim, I_0){
  ...
  for(i in 1:days){
    ...
    if(i == 1){
      ...
      new_time <- not_recovered[, 5] - 1
      not_recovered[, 5] <- new_time
      infected_0 <- new_status[new_status[, 5] == 0]
      infected_0[, 3] <- 'R'
      not_infected = new_status[new_status[, 5] < 0]
