...
infection_status <- function(state, mean_crit_dist,t){
  ...
  #is this right do get the list of recovery days?
  current_rec_time <- state[state[,4]]
  ...
  for(i in 1:nrow(current_infected)){
    for(j in 1:nrow(current_non_infected)){
      ...
      z_s <- as.numeric(current_rec_time[i])
...
simulator <- function(days, num_subjects, x_dim, y_dim, I_0){
  ...
  for(i in 1:days){
    ...
    if(i == 1){
      ...
      new_status <- infection_status(checked_positions, 2,i)
      ...
      new_status <- infection_status(checked_positions, 1/5,i)
      checked_positions[,3] <- new_status
      end_day[[i]] <- checked_positions
