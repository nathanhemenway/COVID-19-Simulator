...
starting_vals <- function(num_subjects, I_0, x_dim, y_dim){
  ...
  #generated days of recovery times for each person
  recovery_days <- rpois(num_subjects,14)
  
  #Put into a data frame
  df <- as.data.frame(cbind(as.numeric(start_x), as.numeric(start_y), subjects,recovery_days))
  colnames(df)[1] <- 'x'
  colnames(df)[2] <- 'y'
  colnames(df)[3] <- 'recovery_times'
  return(df)
}
...
infection_status <- function(state, mean_crit_dist,t){
  ...
  current_recovered <- state[state[,3]=='Rec',]
  ...
  for(i in 1:nrow(current_infected)){
    for(j in 1:nrow(current_non_infected)){
      ...
      z_s <- as.numeric(current_recovered[i,'Rec'])
      ...
      #check recovery time for person at z_s
      if(z_s>=t){
        new_status[j] <- 'Rec'
      }
      else{
